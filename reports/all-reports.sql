...
-- Relatório 3: vendas totais por vendedor/mês

CREATE OR ALTER VIEW VM_SALES_PERSON_MONTH AS (SELECT DISTINCT (VM_PRICES_QUANTITY_BOOK.PRICE * VM_PRICES_QUANTITY_BOOK.QUANTITY) AS TOTAL_SALES,  BOOKS_SALES.SALES_PERSON FROM VM_PRICES_QUANTITY_BOOK INNER JOIN BOOKS_SALES
ON BOOKS_SALES.ID_BOOK = VM_PRICES_QUANTITY_BOOK.BOOK_ID WHERE YEAR(BOOKS_SALES.DATE_SALE) = 2020 AND MONTH(BOOKS_SALES.DATE_SALE) = 10 AND SALES_PERSON = 'Amanda');

SELECT SALES_PERSON, SUM(TOTAL_SALES) AS TOTAL_SALES_CLIENT FROM VM_SALES_PERSON_MONTH GROUP BY SALES_PERSON;
